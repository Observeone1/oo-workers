...
import {
  ...
  validatePayloadHex,
} from './monitor-route-handlers.ts';
...
export function registerMonitorRoutes(app: Hono, deps: RouteDeps): void {
  ...
  app.get('/api/monitors/:type/:id', async (c) => {
    ...
    const result = await getMonitorDetail(type, id);
  ...
  app.post('/api/monitors/api', async (c) => {
    ...
    const assertErr = validateApiAssertions(rawAssertions);
    if (assertErr) return c.json({ error: assertErr }, 400);
  ...
  app.put('/api/monitors/:type/:id', async (c) => {
    ...
    if (!Number.isFinite(id)) return c.json({ error: 'bad id' }, 400);
    const body = await c.req.json();
    const result = await updateMonitorByType(type, id, body);
    return c.json(result.body, result.status);
  });
  ...
  app.post('/api/monitors/:type/:id/run', async (c) => {
    ...
    const id = Number(c.req.param('id'));
    if (!Number.isFinite(id)) return c.json({ error: 'bad id' }, 400);
    const result = await runMonitorNow(type, id, deps);
    return c.json(result.body, result.status);
  });
}
